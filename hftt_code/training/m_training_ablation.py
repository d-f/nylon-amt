...
from model.model_spec2midi_ablation import *
...
if __name__ == '__main__':
    ...
    parser.add_argument('-dropout_convblock', help='dropout parameter for ConvBlock(0.1)', type=float, default=0.1)
    ...
    parser.add_argument('-enc_alg', help='encoder algorithm(CNNtime_SAfreq|CNNblock_SAfreq)', default='CNNtime_SAfreq')
    parser.add_argument('-dec_alg', help='decoder algorithm(CAfreq|CAfreq_SAtime|linear_SAtime)', default='CAfreq_SAtime')
    ...
    print('** AMT(SPEC2MIDI) training for ablation study **')
    ...
    print('  dropout')
    print('   Transformer    : '+str(args.dropout))
    print('   ConvBlock      : '+str(args.dropout_convblock))
    print('  clip            : '+str(args.clip))
    print('  seed            : '+str(args.seed))
    print('  loss weight')
    print('   1st            : '+str(args.weight_A))
    print('   2nd            : '+str(args.weight_B))
    print('  validation')
    ...
    print(' algorithm')
    print('  encoder         : '+str(args.enc_alg))
    print('  decoder         : '+str(args.dec_alg))
    ...
    print('(3-1) encoder')
    if args.enc_alg == 'CNNtime_SAfreq':
        encoder = Encoder_CNNtime_SAfreq(config['input']['margin_b'],
                                         ...
                                         args.dropout,
                                         device)
    elif args.enc_alg == 'CNNblock_SAfreq':
        encoder = Encoder_CNNblock_SAfreq(config['input']['margin_b'],
                                          config['input']['num_frame'],
                                          config['feature']['n_bins'],
                                          args.hid_dim,
                                          args.enc_layer,
                                          ...
                                          args.pf_dim,
                                          args.dropout,
                                          args.dropout_convblock,
                                          device)
    else:
        encoder = None
        print('encoder should be set')

    print('(3-2) decoder and model')
    if args.dec_alg == 'CAfreq':
        decoder = Decoder_CAfreq(config['input']['num_frame'],
                                 config['feature']['n_bins'],
                                 ...
                                 args.dropout,
                                 device)
        model = Model_single(encoder, decoder)
        flag_combination = False
    elif args.dec_alg == 'CAfreq_SAtime':
        decoder = Decoder_CAfreq_SAtime(config['input']['num_frame'],
                                        config['feature']['n_bins'],
                                        config['midi']['num_note'],
                                        config['midi']['num_velocity'],
                                        args.hid_dim,
                                        ...
                                        args.dropout,
                                        device)
        model = Model_combination(encoder, decoder)
        flag_combination = True
    elif args.dec_alg == 'linear_SAtime':
        decoder = Decoder_linear_SAtime(config['input']['num_frame'],
                                        config['feature']['n_bins'],
                                        config['midi']['num_note'],
                                        config['midi']['num_velocity'],
                                        args.hid_dim,
    ...
    parameters = {
        ...
        'training': {
            ...
            'lr': args.lr,
            'dropout': {
                'transformer': args.dropout,
                'convblock': args.dropout_convblock
            },
            'clip': args.clip,
            'seed': args.seed,
        ...
        'algorithm': {
            'encoder': args.enc_alg,
            'decoder': args.dec_alg
        }
    }
    ...
    for epoch in range(epoch_start, args.epoch):
        for div in range(0, args.n_div_train):
            ...
            if flag_combination is True:
                epoch_loss_train = train_ablation.train_combination(model, dataloader_train, optimizer,
                ...
                epoch_loss_train = train_ablation.train_single(model, dataloader_train, optimizer,
                                                               criterion_onset_A, criterion_offset_A, criterion_mpe_A, criterion_velocity_A,
                                                               device, args.v)
            ...
            if args.n_div_valid > 1:
                ...
                for div_valid in range(args.n_div_valid):
                    ...
                    print('## nstep valid: '+str(len(dataloader_valid)))
                    if flag_combination is True:
                        retval = train_ablation.valid_combination(model, dataloader_valid,
                                                                  criterion_onset_A, criterion_offset_A, criterion_mpe_A, criterion_velocity_A,
                                                                  criterion_onset_B, criterion_offset_B, criterion_mpe_B, criterion_velocity_B,
                                                                  args.weight_A, args.weight_B,
                                                                  device)
                    else:
                        retval = train_ablation.valid_single(model, dataloader_valid,
                                                             criterion_onset_A, criterion_offset_A, criterion_mpe_A, criterion_velocity_A,
                                                             device)
                ...
                if flag_combination is True:
                    epoch_loss_valid, num_data_valid = train_ablation.valid_combination(model, dataloader_valid,
                                                                                        criterion_onset_A, criterion_offset_A, criterion_mpe_A, criterion_velocity_A,
                                                                                        criterion_onset_B, criterion_offset_B, criterion_mpe_B, criterion_velocity_B,
                                                                                        args.weight_A, args.weight_B,
                                                                                        device)
                else:
                    epoch_loss_valid, num_data_valid = train_ablation.valid_single(model, dataloader_valid,
                                                                                   criterion_onset_A, criterion_offset_A, criterion_mpe_A, criterion_velocity_A,
                                                                                   device)
            ...
            if args.valid_test is True:
                ...
                if args.n_div_test > 1:
                    ...
                    for div_test in range(args.n_div_test):
                        ...
                        print('## nstep test: '+str(len(dataloader_test)))
                        if flag_combination is True:
                            retval = train_ablation.valid_combination(model, dataloader_test,
                            ...
                            retval = train_ablation.valid_single(model, dataloader_test,
                                                                 criterion_onset_freq, criterion_offset_freq, criterion_mpe_freq, criterion_velocity_freq,
                                                                 device)
                    ...
                    if flag_combination is True:
                        epoch_loss_test, num_data_test = train_ablation.valid_combination(model, dataloader_test,
                                                                                          criterion_onset_A, criterion_offset_A, criterion_mpe_A, criterion_velocity_A,
                                                                                          criterion_onset_B, criterion_offset_B, criterion_mpe_B, criterion_velocity_B,
                                                                                          args.weight_A, args.weight_B,
                                                                                          device)
                    else:
                        epoch_loss_test, num_data_test = train_ablation.valid_single(model, dataloader_test,
                                                                                     criterion_onset_A, criterion_offset_A, criterion_mpe_A, criterion_velocity_A,
                                                                                     device)
