...
def make_dataset(filelist, attribute, d_feature, d_label, d_dataset, config, n_div):
    ...
    a_fname_all = filelist

    for i in tqdm(range(len(a_fname_all))):
        fname = a_fname_all[i]
        div = 0
        if div_flag is True:
            div = i % n_div
        ...
        # num frame
        with open(d_feature.joinpath(fname.stem+'.pkl'), 'rb') as f:
            feature_tmp = pickle.load(f)
        num_frame_feature = feature_tmp.shape[0]
        del feature_tmp
        with open(d_label.joinpath(fname.stem+'.pkl'), 'rb') as f:
    ...
    for div in range(n_div):
        ...
        if div_flag is True:
            ...
            fi = open(d_dataset.joinpath("idx").joinpath(attribute+'.pkl'), 'wb')
    ...
    for div in range(n_div):
        ...
        for i in range(len(a_fname[div])):
            ...
            num_frame = a_num_frame[div][i]
            with open(d_feature.joinpath(a_fname[div][i].stem+'.pkl'), 'rb') as f:
                feature_tmp = pickle.load(f)
            num_frame_feature = feature_tmp.shape[0]
            if config['input']['max_value'] > 0.0:
        ...
        if div_flag is True:
            ...
            ff = open(d_dataset.joinpath('feature').joinpath(attribute+'.pkl'), 'wb')
    ...
    for div in range(n_div):
        ...
        for i in range(len(a_fname[div])):
            ...
            with open(d_label.joinpath(a_fname[div][i].stem+'.pkl'), 'rb') as f:
                label_tmp = pickle.load(f)
            num_frame_label = len(label_tmp['mpe'])
        ...
        if div_flag is True:
            ...
            fl_mpe = open(d_dataset.joinpath('label_mpe').joinpath(attribute+'.pkl'), 'wb')
    ...
    for div in range(n_div):
        ...
        if div_flag is True:
            ...
            fl_onset = open(d_dataset.joinpath('label_onset').joinpath(attribute+'.pkl'), 'wb')
    ...
    for div in range(n_div):
        ...
        if div_flag is True:
            ...
            fl_offset = open(d_dataset.joinpath('label_offset').joinpath(attribute+'.pkl'), 'wb')
    ...
    for div in range(n_div):
        ...
        if div_flag is True:
            ...
            fl_velocity = open(d_dataset.joinpath('label_velocity').joinpath(attribute+'.pkl'), 'wb')
